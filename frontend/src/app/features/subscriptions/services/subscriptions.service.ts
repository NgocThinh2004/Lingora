...
import { SubscriptionAuthor, SubscriptionData, SubscriptionAuthorView } from '../models/subscription.model';
...
export class SubscriptionsService {
  ...
  following(q?: string, page?: number, limit?: number): Observable<{ items: SubscriptionAuthorView[], meta: any }> {
    let params = new HttpParams();
    if (q) params = params.set('q', q);
    if (page) params = params.set('page', page);
    if (limit) params = params.set('limit', limit);
    return this.http
      .get<ApiItemResponse<{ items: SubscriptionAuthorView[], meta: any }>>(`${this.baseUrl}/following`, { params })
      .pipe(map(response => response.data));
  }
